...
{
//VINS中VI-C中Visual Measurement Residual部分所描述的，路标点在非第一次观测处的正切平�?
...
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    double inv_dep_i = parameters[3][0];//逆深�?
    Eigen::Vector3d pts_camera_i = pts_i / inv_dep_i;
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic;//转换到IMU坐标系中
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi;
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj);
    Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic);//前面这几行是arxiv版本公式�?5）第三个公式的实�?
    Eigen::Map<Eigen::Vector2d> residual(residuals);

#ifdef UNIT_SPHERE_ERROR 
    residual =  tangent_base * (pts_camera_j.normalized() - pts_j.normalized());//公式�?5）第一个公式的实现
#else
    ...
    if (jacobians)
    {
        ...
        //求解图像测量误差的雅可比矩阵，包含了对i帧中7camera的位姿，j�?维camera的位姿，7维相机和IMU之间的外参，1维特征点逆深�?        //https://blog.csdn.net/wangshuailpp/article/details/78461171,或者https://blog.csdn.net/JH_233/article/details/80554281有详细的推导
        //此人参考了贺师兄的博客，不懂的可以问问贺师�?
        if (jacobians[0])//对i帧的雅可比矩�?
        {
...
{
    double *res = new double[15];//��ĳһ֡��IMU��״̬������λ�ˣ�7ά�����ٶȡ�ba,bg��15ά,  arXiv�湫ʽ21�еĵڶ�����ʽ
    ...
    Eigen::Matrix<double, 2, 19> num_jacobian;//Ϊ�˲�����ɢ��ʽ����ſɱȾ�����Ҫλ����x,y,z�����һ���Ŷ���һ����3*6+1 = 19�μ���
	//对原始位姿添加扰动后重新计算投影残差及残差变化的雅可比矩�?tmp_residual - residual) / eps
    for (int k = 0; k < 19; k++)
    {
        Eigen::Vector3d Pi(parameters[0][0], parameters[0][1], parameters[0][2]);
        Eigen::Quaterniond Qi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);
